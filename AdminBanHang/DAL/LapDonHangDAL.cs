    ...
    public class LapDonHangDAL
    {
        ...
        public static Product GetProduct(int id)
        {
            using (SqlConnection conn = DBConnection.GetConnection())
            {
                ...
                product.companyid = int.Parse(dt.Rows[0][2].ToString());
        ...
        public static Combo GetCombo(int id)
        {
            using (SqlConnection conn = DBConnection.GetConnection())
            {
                ...
                combo.discountMoney = dt.Rows[0][5].ToString();
                combo.image = dt.Rows[0][6].ToString();
                return combo;
            }
        }
        ...
        public static void AddKhach(string firstname, string lastname, string phone, string address)
        {
            using (SqlConnection conn = DBConnection.GetConnection())
            {
                ...
                string sql = "insert into Customer (Username,Password,Email,FirstName,LastName,Gender,Birthday,Address,Phone,JoinDate,isNew,Status) VALUES ('" +username+"','"+password + "','" + email + "',N'" + firstname
        ...
        public static void AddInvoiceDetail(string products, string combos)
        {
            ...
            {
                conn.Open();
                string sql = "insert into InvoiceDetail(Invoice_Id,Products,Comboes) VALUES ("+getIdInvoice()+",N'" + products +"',N'" + combos + "')";
                SqlCommand command = new SqlCommand(sql, conn);
                command.ExecuteNonQuery();
                conn.Close();
            }
